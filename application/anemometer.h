...
#define ANEMOMETER_TASK_LOOP_DELAY       (10000)
...
//     NUM_HVAC_MODES = 6
// } ANEMOMETER_MODE_T;          // set by user -- do not reorder -- html and ssi.c rely on order
...
//     DUCT_PURGE = 4,
//     ANEMOMETER_LOCKOUT = 5,
//     EXCESSIVE_OVERSHOOT = 6,
//     FAULT_LOCKOUT = 7,
// } ANEMOMETER_STATE_T;         // operational state
...
// anemometer_task.c
void anemometer_task(__unused void *params);
...
// // anemometer_physcial_ui.c
// bool handle_button_press_with_timeout(TickType_t timeout);
// void enable_irq(bool state);
// void gpio_isr(uint gpio, uint32_t events);
// void hvac_update_display(int temperaturex10, ANEMOMETER_MODE_T hvac_mode, int hvac_setpoint);
// int initialize_physical_buttons(int mode_button_gpio, int increase_button_gpio, int decrease_button_gpio);
// ANEMOMETER_MODE_T get_front_panel_mode(void);
// int anemometer_display_initialize(void);
...
// void hvac_log_state_change(ANEMOMETER_STATE_T new_state);
// bool schedule_setpoint_valid(long int temperaturex10, long int heating_temperaturex10, long int cooling_temperaturex10, int mow, ANEMOMETER_MODE_T mode);
// bool schedule_mow_valid(int mow);
// bool schedule_mode_valid(int mode);

// // anemometer_hvac.c
// int initialize_hvac_control(void);
// ANEMOMETER_STATE_T control_anemometer_relays(long int temperaturex10);
